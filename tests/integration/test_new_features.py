...
def test_web_render_text_mode_works(direct_vm, direct_deploy, direct_alice, direct_bob):
    """
    web.render(mode='text') returns a string — confirmed working baseline.
    Used by task_escrow. Validates that mock_web intercepts WebRender calls.
    """
    direct_vm.mock_web(r".*example\.com.*", {"status": 200, "body": "The task is complete."})
    direct_vm.mock_llm(r".*", '{"satisfied": true, "reasoning": "Looks done."}')

    direct_vm.sender = direct_alice
    direct_vm.value = 500
    contract = direct_deploy("contracts/task_escrow.py")
    task_id = contract.create_task("Do the thing", "2099-01-01")
